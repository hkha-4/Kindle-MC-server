...
# Target Kindle performance optimization flags (Cortex-A53 / armv7l)
kindle_flags="-O3 -mcpu=cortex-a53+crypto -mfpu=neon-vfpv4 -mfloat-abi=hard -ffast-math -flto -fomit-frame-pointer"
  ...
  case $arg in
    --9x)
      if [[ "$unameOut" == MINGW64_NT* ]]; then
        ...
        kindle_flags="" # Clear Kindle optimizations if compiling for Win9x
      else
...
# Compile with high-performance settings for the Kindle armhf architecture
$compiler src/*.c $kindle_flags -Iinclude -static -o "bareiron$exe" $windows_linker

echo "Compilation complete! Your Kindle binary is ready."
